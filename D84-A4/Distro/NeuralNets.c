...
void backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS])
{
  ...
  for (int j = 0; j < OUTPUTS; j++) {
    double sum = sumStuff(sample, weights_io, j);
    for (int i = 0; i < INPUTS; i++) {
      // double derivative = activation_derivative(sumStuff(sample, weights_io, j));
      double derivative = derivativeCalc(sigmoid, sum*SIGMOID_SCALE);
...
void backprop_2layer(double sample[INPUTS],double h_activations[MAX_HIDDEN], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS], int units)
{
  ...
  for (int j = 0; j < OUTPUTS; j++) {
    ...
    sums_l[j] = sum;
    for (int i = 0; i < units; i++){
      double derivative = derivativeCalc(sigmoid, (SIGMOID_SCALE*(MAX_HIDDEN/units))*sum);
  ...
  for (int j = 0; j < units; j++) {
    ...
    for (int i = 0; i < INPUTS; i++){
      ...
      for (int k = 0; k < OUTPUTS; k++) {
        ec += weights_ho[j][k] * ((double)(k==label) - activations[k]) * derivativeCalc(sigmoid, sums_l[k]*(SIGMOID_SCALE*(MAX_HIDDEN/units)));
      }
...
double sumStuff3(double sample[INPUTS], double weights_io[MAX_HIDDEN][OUTPUTS], int output, int units)
{
  double sum = 0.0;
  for (int i = 0; i < units; i++) {
    sum += sample[i]*weights_io[i][output];
...
double sumStuff2(double sample[INPUTS], double weights_ih[INPUTS][MAX_HIDDEN], int output)
{
  double sum = 0.0;
  for (int i = 0; i < INPUTS; i++) {
    sum += sample[i]*weights_ih[i][output];
  }
  return sum;
}
...
double derivativeCalc(double (*sigmoid)(double input), double input) {
  double h = 0.00002;
  return (sigmoid(input + h) - sigmoid(input))/h;
}

double dTanh2(double input, int units) {
  return 1 - pow(tanh(input*SIGMOID_SCALE*(MAX_HIDDEN*units)), 2);
}

double dLog2(double input, int units) {
  // return (exp(-1*input)/ (pow(1 + exp(-1*input), 2)));
  double s = logistic(input*SIGMOID_SCALE*(MAX_HIDDEN/units));
  return s * (1 - s);
}